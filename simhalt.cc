	...
	if(cargo[catg])
	{
		...
		for(int j = packet_count - 1; j  >= 0; j--)
		{
			...
			if(ware.is_passenger()
			   && ware.get_zwischenziel().is_bound()
...
void haltestelle_t::get_destination_halts_of_ware(ware_t &ware, vector_tpl<halthandle_t>& destination_halts_list) const
{
				...
				if(haltlist_count)
				{
					...
					for(int i = 0; i < haltlist_count; i++)
					{
						if (!haltlist[i].halt.is_bound())
						{
							...
							DBG_MESSAGE("haltestelle_t::get_destination_halts_of_ware()", "DIAG: unbound halt in haltlist of plan %s (i=%d of %d, entry=%u)", plan->get_kartenboden()->get_pos().get_str(), i, haltlist_count, haltlist[i].halt.get_id());
							continue;
						}
	...
	else
	{
		...
		for(uint16 h = 0; h < plan->get_haltlist_count(); h++)
		{
			...
			{
				// Defensive guard against unbound halt list entries (which karte_t::load purges at load).
				DBG_MESSAGE("haltestelle_t::get_destination_halts_of_ware()", "DIAG: unbound halt in haltlist of single-tile plan %s (h=%u of %u, entry=%u)", plan->get_kartenboden()->get_pos().get_str(), h, plan->get_haltlist_count(), halt.get_id());
				continue;
			}
