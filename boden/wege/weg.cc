...
void weg_t::private_car_route_map::diagnose_memory_usage()
{
	for (uint8 elem = 0; elem < 2; elem++)
	{
		uint32 road_ways = 0;
		uint32 slots_nonempty = 0;
		uint32 slots_single = 0;
		uint32 slots_master = 0;
		uint32 slots_linked = 0;
		uint64 logical_entries = 0;
		FOR(vector_tpl<weg_t*>, const w, alle_wege)
		{
			if (w->get_waytype() != road_wt)
			{
			...
			for (uint8 d = 0; d < 5; d++)
			{
				...
				slots_nonempty++;
				logical_entries += n;
				if (m.link_mode == link_mode_single)
				{
					slots_single++;
				}
				else if (m.link_mode == link_mode_master)
				{
					slots_master++;
				}
				else
		...
		uint64 stored_entries = 0;
		uint32 stored_shared = 0;
		const uint32 stored_lists = route_maps[elem].get_count();
		for (uint32 i = 0; i < stored_lists; i++)
		{
			stored_entries += route_maps[elem][i].destinations.get_count();
			if (route_maps[elem][i].shared)
			{
				stored_shared++;
			}
		}
		// Each entry is one koord (4 bytes). Each stored list object is 16 bytes
		// (pointer, capacity, count) inside the route_maps vector, plus a heap
		// allocation whose capacity is at most twice its entry count (doubling
		// growth), so the true byte figures are at most ~2x the entry bytes shown.
		dbg->warning("pcrm memory", "EXPERIMENT element %u (%s): road_ways=%u nonempty_slots=%u (single=%u master=%u linked=%u) logical_entries=%llu stored_lists=%u (shared=%u) stored_entries=%llu stored_bytes~%llu unshared_bytes~%llu",
			elem,
			elem == weg_t::private_car_routes_currently_reading_element ? "reading" : "writing",
			road_ways, slots_nonempty, slots_single, slots_master, slots_linked,
			(unsigned long long)logical_entries, stored_lists, stored_shared, (unsigned long long)stored_entries,
			(unsigned long long)(stored_entries * 4 + (uint64)stored_lists * 16),
			(unsigned long long)(logical_entries * 4 + (uint64)(slots_nonempty - slots_single) * 16));
	}
}
