...
{
	return world()->get_inflation_adjusted_price(world()->get_timeline_year_month(), obj_desc_transport_related_t::get_value(), infrastructure);
}

sint64 obj_desc_transport_infrastructure_t::get_base_way_only_cost() const
{
	return world()->get_inflation_adjusted_price(world()->get_timeline_year_month(), obj_desc_transport_related_t::get_base_way_only_cost(), infrastructure);
}

sint64 obj_desc_transport_infrastructure_t::get_way_only_cost() const
{
	return world()->get_inflation_adjusted_price(world()->get_timeline_year_month(), obj_desc_transport_related_t::get_way_only_cost(), infrastructure);
}
