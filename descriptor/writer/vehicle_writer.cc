...
void vehicle_writer_t::write_obj(FILE* fp, obj_node_t& parent, tabfileobj_t& obj)
{
	...
	// Price of this vehicle in Simucents
	...
	*
	...
	for (i = 0; i < 8; i++)
	{
		...
		// Empty vehicle image for direction, direction in "s", "w", "sw", "se", asymmetric vehicles need also "n", "e", "ne", "nw"
		if (livery_max == 0)
		{
	...
	do {
		...
		// Constraints for previous vehicles
		// val: string, "none" means only suitable at front of an convoi
	...
	do {
		...
		// Constraints for successing vehicles
		// val: string, "none" to disallow any followers
