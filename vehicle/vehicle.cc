...
void vehicle_t::overhaul()
{
	...
	last_maintenance_month = welt->get_current_month();
	last_maintenance_time = welt->get_ticks();
	cnv->set_wait_lock((welt->ticks_per_world_month * desc->get_overhaul_month_tenths()) / 10u);  // TODO: Deal with multiple vehicles in a convoy being overhauled and the impact on timings of this.
