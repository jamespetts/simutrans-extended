				...
				for (uint8 i = 0; i < classes_amount; i++)
				{
					if (veh_type->get_capacity(i) > 0)
					{
						...
						if (pass_veh)
						{
							...
							welt->sprintf_time_secs(timebuf, sizeof(timebuf), welt->get_settings().max_tolerable_journey(base_comfort + modified_comfort), (env_t::show_month==env_t::DATE_FMT_64_SECOND_MINUTE || env_t::show_month==env_t::DATE_FMT_64_SECOND_MINUTE_PRETTY) );
