	...
	FOR(vector_tpl<image_data_t*>, const& iptr, *images) {
		...
		if(idata.count>=0) {
			...
			if(idata.lcolor!=EMPTY_IMAGE_BAR) {
				display_veh_form_wh_clip_rgb(xpos + 1, ypos + grid.y - VEHICLE_BAR_HEIGHT - 1, grid.x/2 - 1, VEHICLE_BAR_HEIGHT, idata.lcolor, true, idata.basic_coupling_constraint_prev, idata.interactivity, false);
			}
			if(idata.rcolor!=EMPTY_IMAGE_BAR) {
				display_veh_form_wh_clip_rgb(xpos + grid.x/2, ypos + grid.y - VEHICLE_BAR_HEIGHT - 1, grid.x - grid.x/2 - 1, VEHICLE_BAR_HEIGHT, idata.rcolor, true, idata.basic_coupling_constraint_next, idata.interactivity, true);
			}
			if (sel_index-- == 0) {
