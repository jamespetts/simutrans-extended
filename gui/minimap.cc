...
const fabrik_t* minimap_t::draw_factory_connections(const fabrik_t* const fab, bool supplier_link, const scr_coord pos) const
{
	if(fab) {
		...
		auto lieferziele = supplier_link ? fab->get_suppliers() : fab->get_consumers();
		for(auto lieferziel : lieferziele) {
			...
			if (fab2) {
				...
				display_ddd_proportional_clip(boxpos.x, boxpos.y, supplier_link ? color_idx_to_rgb(COL_DODGER_BLUE-1) : color_idx_to_rgb(COL_ORANGE-1), color_idx_to_rgb(COL_WHITE), name, true);
			}
		}
	}
	return fab;
