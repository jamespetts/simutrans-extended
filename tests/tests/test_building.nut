...
function test_building_build_house()
{
	...
	{
		ASSERT_EQUAL(builder.work(public_pl, coord3d(0, 0, 0), "11#Small_Football_Ground"), null)
		local b = building_x(0, 0, 0)
		ASSERT_EQUAL(b.get_owner().get_name(), public_pl.get_name())
		ASSERT_EQUAL(b.get_desc().get_name(), "Small_Football_Ground")
...
function test_building_build_multi_tile_sloped()
{
	...
	local building_desc = building_desc_x("ParishChurchHolyTrinity") // 2x3 size

	ASSERT_EQUAL(raise.work(pl, coord3d(4, 2, 0)), null)

	// FIXME: In Standard, this is allowed and automatically terraforms the terrain.
	{
		ASSERT_EQUAL(builder.work(pl, coord3d(3, 1, 0), "112" + building_desc.get_name()), "No suitable ground!")
	}
...
function test_building_rotate_house()
{
	...
	ASSERT_EQUAL(builder.work(public_pl, coord3d(0, 0, 0), "011ParishChurchHolyTrinity"), null)
...
function test_building_rotate_station()
{
	...
	local layout2_station = building_desc_x("GoodsSiding")     // this one has only 2 layouts
	local layout16_station = building_desc_x("BulkLoadingOld") // this one has 16 layouts
...
function test_building_rotate_factory()
{
	...
	// non-square building with 4 rotations (There are no multi-tile buildings in pak128.britain-ex with only 2 rotations)
	ASSERT_EQUAL(build_factory(public_pl, coord3d(3, 4, 0), 0, 0, production, "Dairy1910"), null)
	ASSERT_TRUE(tile_x(3, 4, 0).find_object(mo_building) != null)
	ASSERT_TRUE(tile_x(4, 4, 0).find_object(mo_building) != null)

	{
		ASSERT_EQUAL(rotator.work(public_pl, coord3d(3, 4, 0)), null)
		ASSERT_TRUE(tile_x(3, 4, 0).find_object(mo_building) != null)
		ASSERT_TRUE(tile_x(4, 4, 0).find_object(mo_building) != null)
